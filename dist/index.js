...
var __dname = typeof __dirname !== "undefined" ? __dirname : path.dirname(fileURLToPath(import.meta?.url));
var resolveInclusions = async () => {
  try {
    ...
    const xsltContent = await fs2.promises.readFile(xsltFile, "utf8");
    ...
    for (const node of includeNodes) {
      ...
      if (href) {
        ...
        const data = await fs2.promises.readFile(includePath, "utf8");
        const includeDoc = new DOMParser().parseFromString(data, "application/xml");
...
var { parser } = pkg;
var { streamArray } = pakg;
var __dname2 = typeof __dirname !== "undefined" ? __dirname : path2.dirname(fileURLToPath2(import.meta?.url));
var CatalogoSat = class {
  ...
  searchCatalogo(name) {
    try {
      const json_file = path2.join(__dname2, "resources", `catalogos/${name.toLowerCase()}.json`);
      if (fs3.existsSync(json_file)) {
  ...
  search(key, value) {
    return new Promise((resolve, reject) => {
      ...
      const data = fs3.createReadStream(this.catPath).pipe(parser()).pipe(streamArray());
      let found = false;
      data.on("data", (chunk) => {
        if (!chunk.value[key]) {
...
export {
  CartaPorteAereo_default as CartaPorteAereo,
  CartaPorteAutotransporte_default as CartaPorteAutotransporte,
  CartaPorteFerroviario_default as CartaPorteFerroviario,
  CartaPorteMaritimo_default as CartaPorteMaritimo,
  CatalogoSat_default as CatalogoSat,
  ConfigCfdi_default as ConfigCfdi,
  FacturaCfdi_default as FacturaCfdi,
  Pago_default as Pago,
